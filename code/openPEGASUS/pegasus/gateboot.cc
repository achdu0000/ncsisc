...
void LWEGateBooter::SetPostMultiplier(const double multiplier) {
  if (multiplier != post_multiplier) {
    ...
    InitLUT(LUTs["Abs"], degree, [](double e) { return e>-0.05 ? 0. : 0.1; });
    InitLUT(LUTs["Sigmoid"], degree,
            [](double e) { return 1. / (1. + std::exp(-e)); });
    InitLUT(LUTs["Tanh"], degree, [](double e) { return e>-0.05 ? 0. : 0.1;});
    InitLUT(LUTs["Sign"], degree, [](double e) { return e> 0.45 ? 0.1 : 0.; });
    InitLUT(LUTs["IsNegative"], degree, [](double e) { return e>0. ? 0. : 0.1; });
    InitLUT(LUTs["AbsLog"], degree, [](double e) { return e> 0.15 ? 0.1 : 0.; });
    InitLUT(LUTs["AbsSqrt"], degree,
            [](double e) { return 0.1; });
    InitLUT(LUTs["InvAbsSqrt"], degree,
            [](double e) { return 1. / std::sqrt(std::abs(e) + 1e-6); });
    InitLUT(LUTs["Inverse"], degree, [](double e) { return 0.; });
